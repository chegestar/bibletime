 ...
 */
/* This original code is commented out due to the conversion changes */
//static int FtpOpenPort(const char *host, netbuf *nControl, netbuf **nData, int mode, int dir)
...
//		printf("short write: passed %d, wrote %d\n", l, c);
    ...
    else
    {
    	...
    	{
		rvstmp = (char *)calloc(strlen(rvs) + strlen(dbuf) + 1, sizeof(char));

		strcpy(rvstmp, rvs);
		strcat(rvstmp, dbuf);
  
		rvs = (char *)calloc(strlen(rvstmp) + 1, sizeof(char));
...
char* FtpNlst(const char *outputfile, const char *path,
	netbuf *nControl)
{
/* This original code is commented out due to the conversion changes */
...
char* FtpPut(const char *inputfile, const char *path, char mode,
	netbuf *nControl)
{
/* This original code is commented out due to the conversion changes */
//  return FtpXfer(inputfile, path, nControl, FTPLIB_FILE_WRITE, mode);
    FtpXfer(inputfile, path, nControl, FTPLIB_FILE_WRITE, mode);
    return rvs;
}
...
char* FtpDelete(const char *fnm, netbuf *nControl)
{
    char cmd[256];
